...
namespace MVC.Pages.Issuance
{
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            // Use fully-qualified domain types to avoid name conflicts with this page's namespace
            var issuance = new Hassann_Khala.Domain.Issuance { SerialNumber = SerialNumber ?? Guid.NewGuid().ToString("N"), CreatedAt = DateTime.UtcNow };
            for (int i = 0; i < Items.Count; i++)
            {
                issuance.Items.Add(new Hassann_Khala.Domain.IssuanceItem { ItemIndex = i, FieldValue = Items[i].FieldValue, ProductId = Items[i].ProductId, SectionId = Items[i].SectionId });
            }
